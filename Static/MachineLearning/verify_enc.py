...
d={}
def initializare(d):
    data = open( './MachineLearning/probabil.txt' ).read().split('\n')
...
def initializare_ads(d):
    data = open( './MachineLearning/ads_prob.txt' ).read().split('\n')
    for i in range(len(data)):
        try:
            data[i]=data[i].split(':')
            d[data[i][0]]=data[i][1]
...
def initializare_DNS(d):
    data = open( './MachineLearning/probabilitati_dns.txt' ).read().split('\n')
    for i in range(len(data)):
        try:
            data[i]=data[i].split(':')
            d[data[i][0]]=data[i][1]
...
def initializare_big():
    data = open( './MachineLearning/probabilitati_subdomenii.txt' ).read().split('\n')
    for i in range(len(data)):
        try:
            data[i]=data[i].split(':')
            d[data[i][0]]=data[i][1]
...
def initializare_extins():
    data = open( './MachineLearning/probabilitati_extins.txt' ).read().split('\n')
    for i in range(len(data)):
        try:
            data[i]=data[i].split(':')
            d[data[i][0]]=data[i][1]
