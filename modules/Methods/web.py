...
def http_start():
	...
	while(start_http!=0):
		...
		added=0
		added_GET=0
		cmd="sudo tcpdump -i ens33 -A -s 0 'tcp dst port 80 and (((ip[2:2] - ((ip[0]&0xf)<<2)) - ((tcp[12]&0xf0)>>2)) != 0)'  -c 1"
		founda=0
		...
		for i in range(len(result)):
			...
			if 'Agent' in result[i][0]:
				if result[i][1] != 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:84.0) Gecko/20100101 Firefox/84.0':
