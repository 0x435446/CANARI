								...
								if c[i].words[c[i].index] in data[1].decode():
									...
									if (hashlib.sha256(data[1]).hexdigest() == data[0].decode()):
										if data[0].decode()[:3] == '000':
											...
											#cursor.execute("INSERT INTO alerte (Type,Message,Risk,Destination,Payload,Timestamp) VALUES('Listening', '"+ data[2].split('-')[0].encode()+" "+data[2].split('-')[2].encode()+"','MEDIUM','"+ data[2].split('-')[1].encode() +"','-','"+str(datetime.now()).encode()+"')")
											db.commit()
											print ("AM AJUNS")
											connection.sendall(random_bytes.encode())
											connection.close()
